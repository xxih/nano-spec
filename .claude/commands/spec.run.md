...
如 `/run 实现用户登录功能，支持手机号和邮箱两种方式`

判断标准：输入包含较完整的功能描述（超过 10 个字）

执行流程：
1. 从需求描述中提取简短标题（如「用户登录」）
2. **自动执行 CLI 命令**创建目录（若不存在）：
   ```bash
   nanospec new 用户登录
   ```
3. 将需求描述写入 `brief.md`（覆盖模板内容）
4. 直接从 1-spec 开始执行，**无需任何用户干预**

**场景 B：无需求描述**

如 `/run` 或 `/run 20260202-用户登录`

执行流程：
1. 确定工作目录：
   - 用户显式指定的目录/任务名
   - 或从 `<config_dir>/.current` 读取
2. 如果目录不存在或无效 → 提示用户提供需求描述：
   ```
   未找到有效任务目录。请使用以下方式之一：
   1. /run 你的需求描述（将自动创建任务并执行）
   2. nanospec new 任务名（手动创建后再执行 /run）
   ```
3. 如果目录有效 → 检测进度并继续执行
...
```
场景 A（有需求描述）→ 创建目录 + 写入 brief，从 1-spec 开始
场景 B + 无效目录  → 提示用户提供需求描述
场景 B + 无 brief  → 从 1-spec 开始（使用模板 brief）
场景 B + 有 brief  → 从 1-spec 开始
场景 B + 有 spec   → 从 2-plan 开始
场景 B + 有 plan   → 从 3-execute 开始
场景 B + 有 tasks  → 继续执行未完成任务
全部完成           → 提示工作流已完成
```
