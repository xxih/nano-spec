...
根据当前环境状态执行不同操作：
1. **未初始化**：执行 `specflow init` 初始化目录结构
2. **已初始化**：执行 `specflow new` 创建任务，并根据用户意图填写 brief.md
...
1. 执行 shell 命令：`specflow init --tools cursor`
...
1. 从用户输入中提取任务名称和意图
2. 执行 shell 命令：`specflow new "<任务名称>"`
...
3. 提示用户下一步使用 `/spec.1-spec` 生成规格说明
