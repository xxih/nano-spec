...
#### 2.1.1 nanospec init 命令

**描述**：初始化 nanospec 项目结构
...
- [ ] 执行后在项目根目录创建 `nanospec/` 目录
- [ ] `nanospec/AGENTS.md` 文件存在且内容完整
- [ ] `nanospec/templates/` 目录包含 6 个产出物模板文件
...
```
✓ 创建 nanospec/AGENTS.md
✓ 创建 nanospec/templates/ (6 个模板)
✓ 创建 .cursor/commands/ (6 个命令)

🎉 nanospec 初始化完成！

下一步：
  1. nanospec new "任务名称"  创建任务目录
  2. 编辑 brief.md 描述需求
...
- [ ] 创建目录 `nanospec/YYYYMMDD-<名称>/`
- [ ] 目录包含：`brief.md`、`assets/`、`outputs/`
- [ ] `brief.md` 包含任务名称标题和占位符文本
- [ ] 若未指定名称，使用"待命名"作为默认值
- [ ] 若目录已存在，显示警告并退出
- [ ] 若未执行 `nanospec init`，显示错误信息

**输出示例**：

```
✓ 创建任务目录: nanospec/20260120-用户认证功能/
  ├── brief.md
  ├── assets/
  └── outputs/
...
- [ ] 每个模板文件存在且格式正确
- [ ] 模板包含必要的字段和结构说明
- [ ] 模板从 `assets/templates/` 复制到 `nanospec/templates/`
...
- [ ] `npm install -g nanospec-cli` 后可直接运行 `nanospec` 命令
- [ ] bin/nanospec.js 正确引导到 dist/index.js
