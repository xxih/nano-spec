...
nanospec 只支持两种定制方式，保持极简：

### 3.1 修改模板（templates/）

模板决定产出物的结构和格式。修改 `nanospec/templates/*.md` 即可定制：
...
| 文档                 | 作用                                       |
| -------------------- | ------------------------------------------ |
| `nanospec/AGENTS.md` | 通用规则（强制规则、产物约定、对齐格式等） |
...
| 命令                        | 功能         | 说明                               |
| --------------------------- | ------------ | ---------------------------------- |
| `nanospec init`             | 初始化项目   | 创建目录结构、模板、AGENTS.md      |
| `nanospec init --ai <tool>` | 适配指定 AI  | 生成对应 AI 工具的命令文件         |
| `nanospec new`              | 创建任务目录 | 创建 `YYYYMMDD-待命名/` 壳子文件夹 |

### 5.2 new 命令详解

```bash
nanospec new
# 创建: nanospec/20260119-待命名/
#       ├── brief.md          # 空文件
#       ├── assets/           # 空目录
#       └── outputs/          # 空目录

nanospec new "用户认证功能"
# 创建: nanospec/20260119-用户认证功能/
```

目录结构：

```
nanospec/YYYYMMDD-<名称>/
...
- [ ] `nanospec` CLI 工具（init 命令）
- [ ] `nanospec/AGENTS.md` - 通用规则
- [ ] `nanospec/templates/` - 6 个产出物模板
