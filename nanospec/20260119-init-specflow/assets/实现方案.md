# nanospec CLI 实现方案

> 技术栈：TypeScript + Node.js（极简版，目标 ~300 行）

---

## 1. 项目结构

```
nanospec-cli/
├── package.json
├── tsconfig.json
├── src/
│   ├── index.ts              # CLI 入口
│   ├── commands/
│   │   ├── init.ts           # nanospec init
│   │   └── new.ts            # nanospec new
│   ├── adapters/             # AI 适配器
│   │   ├── index.ts          # 适配器注册
│   │   ├── cursor.ts         # Cursor 命令生成
│   │   └── claude.ts         # Claude 命令生成（待实现）
│   └── templates/            # 内置模板
│       ├── AGENTS.md
│       ├── commands/
│       │   ├── flow.1-spec.md
│       │   ├── flow.2-plan.md
│       │   ├── flow.3-execute.md
│       │   ├── flow.accept.md
│       │   ├── flow.align.md
│       │   └── flow.summary.md
│       └── outputs/
│           ├── 1-spec.md
│           ├── 2-plan.md
│           ├── 3-tasks.md
│           ├── acceptance.md
│           ├── alignment.md
│           └── summary.md
└── bin/
    └── nanospec.js
```

---

## 2. CLI 入口 (`src/index.ts`)

```typescript
#!/usr/bin/env node

import {Command} from 'commander';
import {init} from './commands/init';
import {newTask} from './commands/new';

const program = new Command();

program
	.name('nanospec')
	.description('nanospec - Spec 驱动开发工作流')
	.version('1.0.0');

// nanospec init
program
	.command('init')
	.description('初始化 nanospec 项目结构')
	.option('--ai <tool>', 'AI 工具类型', 'cursor')
	.option('-f, --force', '强制覆盖已存在的文件')
	.action((options) => init(options));

// nanospec new
program
	.command('new [name]')
	.description('创建新的任务目录')
	.action((name) => newTask(name));

program.parse();
```

---

## 3. init 命令 (`src/commands/init.ts`)

```typescript
import {existsSync, mkdirSync, writeFileSync, cpSync} from 'fs';
import {join, dirname} from 'path';
import {fileURLToPath} from 'url';
import {getAdapter} from '../adapters';

interface InitOptions {
	ai: string;
	force?: boolean;
}

export async function init(options: InitOptions): Promise<void> {
	const cwd = process.cwd();
	const nanospecDir = join(cwd, 'nanospec');
	const templatesDir = join(nanospecDir, 'templates');

	// 检查是否已存在
	if (existsSync(nanospecDir) && !options.force) {
		console.log('⚠️  nanospec/ 目录已存在，使用 --force 强制覆盖');
		return;
	}

	// 获取 AI 适配器
	const adapter = getAdapter(options.ai);
	if (!adapter) {
		console.log(`❌ 不支持的 AI 工具: ${options.ai}`);
		console.log('   支持: cursor, claude');
		return;
	}

	// 创建目录结构
	mkdirSync(templatesDir, {recursive: true});

	// 复制内置模板
	const __dirname = dirname(fileURLToPath(import.meta.url));
	const builtinTemplates = join(__dirname, '../templates');

	// 复制 AGENTS.md
	copyFile(join(builtinTemplates, 'AGENTS.md'), join(nanospecDir, 'AGENTS.md'));
	console.log('✓ 创建 nanospec/AGENTS.md');

	// 复制产出物模板
	const outputTemplates = [
		'1-spec.md',
		'2-plan.md',
		'3-tasks.md',
		'acceptance.md',
		'alignment.md',
		'summary.md'
	];
	for (const template of outputTemplates) {
		copyFile(
			join(builtinTemplates, 'outputs', template),
			join(templatesDir, template)
		);
	}
	console.log('✓ 创建 nanospec/templates/ (6 个模板)');

	// 生成 AI 命令文件
	adapter.generateCommands(cwd, builtinTemplates);
	console.log(`✓ 创建 ${adapter.commandsDir} (6 个命令)`);

	console.log('\n🎉 nanospec 初始化完成！');
	console.log('\n下一步：');
	console.log('  1. nanospec new "任务名称"  创建任务目录');
	console.log('  2. 编辑 brief.md 描述需求');
	console.log('  3. 使用 /flow.1-spec 开始规格撰写');
}

function copyFile(src: string, dest: string): void {
	const dir = dirname(dest);
	if (!existsSync(dir)) {
		mkdirSync(dir, {recursive: true});
	}
	if (existsSync(src)) {
		cpSync(src, dest);
	}
}
```

---

## 4. new 命令 (`src/commands/new.ts`)

```typescript
import {existsSync, mkdirSync, writeFileSync} from 'fs';
import {join} from 'path';

export async function newTask(name?: string): Promise<void> {
	const cwd = process.cwd();
	const nanospecDir = join(cwd, 'nanospec');

	// 检查是否已初始化
	if (!existsSync(nanospecDir)) {
		console.log('❌ 请先运行 nanospec init 初始化项目');
		return;
	}

	// 生成目录名：YYYYMMDD-名称
	const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
	const taskName = name || '待命名';
	const dirName = `${date}-${taskName}`;
	const taskDir = join(nanospecDir, dirName);

	// 检查是否已存在
	if (existsSync(taskDir)) {
		console.log(`⚠️  目录已存在: ${dirName}`);
		return;
	}

	// 创建目录结构
	mkdirSync(join(taskDir, 'assets'), {recursive: true});
	mkdirSync(join(taskDir, 'outputs'), {recursive: true});

	// 创建空的 brief.md
	writeFileSync(
		join(taskDir, 'brief.md'),
		`# ${taskName}\n\n<!-- 在此描述需求 -->\n`,
		'utf-8'
	);

	console.log(`✓ 创建任务目录: nanospec/${dirName}/`);
	console.log('  ├── brief.md');
	console.log('  ├── assets/');
	console.log('  └── outputs/');
	console.log('\n下一步：编辑 brief.md 描述需求，然后使用 /flow.1-spec');
}
```

---

## 5. AI 适配器

### 5.1 适配器接口 (`src/adapters/index.ts`)

```typescript
export interface AIAdapter {
	name: string;
	commandsDir: string;
	generateCommands(cwd: string, templatesDir: string): void;
}

import {cursorAdapter} from './cursor';
import {claudeAdapter} from './claude';

const adapters: Record<string, AIAdapter> = {
	cursor: cursorAdapter,
	claude: claudeAdapter
};

export function getAdapter(name: string): AIAdapter | null {
	return adapters[name] || null;
}

export function listAdapters(): string[] {
	return Object.keys(adapters);
}
```

### 5.2 Cursor 适配器 (`src/adapters/cursor.ts`)

```typescript
import {existsSync, mkdirSync, cpSync} from 'fs';
import {join} from 'path';
import type {AIAdapter} from './index';

export const cursorAdapter: AIAdapter = {
	name: 'cursor',
	commandsDir: '.cursor/commands/',

	generateCommands(cwd: string, templatesDir: string): void {
		const commandsDir = join(cwd, '.cursor', 'commands');
		mkdirSync(commandsDir, {recursive: true});

		const commands = [
			'flow.1-spec.md',
			'flow.2-plan.md',
			'flow.3-execute.md',
			'flow.accept.md',
			'flow.align.md',
			'flow.summary.md'
		];

		for (const cmd of commands) {
			const src = join(templatesDir, 'commands', cmd);
			const dest = join(commandsDir, cmd);
			if (existsSync(src)) {
				cpSync(src, dest);
			}
		}
	}
};
```

### 5.3 Claude 适配器 (`src/adapters/claude.ts`)

```typescript
import {existsSync, mkdirSync, readFileSync, writeFileSync} from 'fs';
import {join} from 'path';
import type {AIAdapter} from './index';

export const claudeAdapter: AIAdapter = {
	name: 'claude',
	commandsDir: '.claude/commands/',

	generateCommands(cwd: string, templatesDir: string): void {
		const commandsDir = join(cwd, '.claude', 'commands');
		mkdirSync(commandsDir, {recursive: true});

		// TODO: 参考 spec-kit 实现 Claude 命令格式转换
		// Claude 命令格式可能与 Cursor 不同，需要适配

		const commands = [
			'flow.1-spec.md',
			'flow.2-plan.md',
			'flow.3-execute.md',
			'flow.accept.md',
			'flow.align.md',
			'flow.summary.md'
		];

		for (const cmd of commands) {
			const src = join(templatesDir, 'commands', cmd);
			const dest = join(commandsDir, cmd);
			if (existsSync(src)) {
				// 读取并转换格式（如需要）
				const content = readFileSync(src, 'utf-8');
				const converted = convertToClaude(content);
				writeFileSync(dest, converted, 'utf-8');
			}
		}
	}
};

function convertToClaude(content: string): string {
	// TODO: 实现 Cursor → Claude 命令格式转换
	// 参考 spec-kit 代码仓库
	return content;
}
```

---

## 6. 配置文件

### 6.1 package.json

```json
{
	"name": "nanospec-cli",
	"version": "1.0.0",
	"description": "nanospec - Spec 驱动开发工作流",
	"type": "module",
	"main": "dist/index.js",
	"bin": {
		"nanospec": "./bin/nanospec.js"
	},
	"scripts": {
		"build": "tsc",
		"dev": "tsx src/index.ts"
	},
	"dependencies": {
		"commander": "^12.0.0"
	},
	"devDependencies": {
		"@types/node": "^22.0.0",
		"typescript": "^5.5.0",
		"tsx": "^4.0.0"
	},
	"engines": {
		"node": ">=18"
	}
}
```

### 6.2 tsconfig.json

```json
{
	"compilerOptions": {
		"target": "ES2022",
		"module": "NodeNext",
		"moduleResolution": "NodeNext",
		"outDir": "dist",
		"rootDir": "src",
		"strict": true,
		"esModuleInterop": true,
		"skipLibCheck": true,
		"declaration": true
	},
	"include": ["src/**/*"],
	"exclude": ["node_modules", "dist"]
}
```

### 6.3 bin/nanospec.js

```javascript
#!/usr/bin/env node
import '../dist/index.js';
```

---

## 7. 使用示例

```bash
# 安装（全局）
npm install -g nanospec-cli

# 初始化项目（默认 Cursor）
nanospec init

# 指定 AI 工具
nanospec init --ai claude

# 强制覆盖
nanospec init --force

# 创建任务目录
nanospec new                    # → nanospec/20260119-待命名/
nanospec new "用户认证功能"      # → nanospec/20260119-用户认证功能/
```

---

## 8. 内置模板清单

### 8.1 AGENTS.md

基于现有 `nanospec/AGENTS.md`，包含：

- 变量定义
- 强制规则
- 目录结构
- 输入优先级
- 输出模板
- 变更传播
- 任务状态
- 命令概览
- 定制指南

### 8.2 斜杠命令（6 个）

| 文件                | 职责            |
| ------------------- | --------------- |
| `flow.1-spec.md`    | 规格撰写        |
| `flow.2-plan.md`    | 方案 + 任务拆解 |
| `flow.3-execute.md` | 执行交付        |
| `flow.accept.md`    | 验收用例        |
| `flow.align.md`     | 对齐纠偏        |
| `flow.summary.md`   | 总结沉淀        |

### 8.3 产出物模板（6 个）

| 文件            | 用途         |
| --------------- | ------------ |
| `1-spec.md`     | 规格文档结构 |
| `2-plan.md`     | 技术方案结构 |
| `3-tasks.md`    | 任务清单格式 |
| `acceptance.md` | 验收用例格式 |
| `alignment.md`  | 对齐记录格式 |
| `summary.md`    | 总结文档格式 |

---

## 9. 后续迭代

| 优先级 | 功能                  | 说明                 |
| ------ | --------------------- | -------------------- |
| P1     | init + new            | 核心命令             |
| P1     | Cursor 适配器         | 默认支持             |
| P2     | Claude 适配器         | 参考 spec-kit        |
| P2     | GitHub Copilot 适配器 | 待调研命令格式       |
| P3     | 模板变量替换          | 支持 `{{date}}` 等   |
| P3     | 配置文件              | 可选的 `.nanospecrc` |
