...
在 `.iflow/commands/` 目录下创建：

- `spec.ralph-status.toml` - 查看当前任务状态
- `spec.ralph-add.toml` - 添加新任务
- `spec.ralph-sync.toml` - 同步 prd.json 状态
- `spec.ralph-complete.toml` - 标记当前任务为完成

#### 3.2 命令示例

`spec.ralph-status.toml`:
```toml
# Command: spec.ralph-status
# Description: 查看 Ralph 当前任务状态

description = "查看 Ralph 当前任务状态"

prompt = """
读取 nanospec/ralph/prd.json，以表格形式展示任务列表，包括：
- 任务 ID
- 标题
- 状态
- 当前阶段
