...
export const windsurfAdapter: AIAdapter = {
  ...
  generateCommands(cwd: string, templatesDir: string): void {
    const commandsDir = join(cwd, '.windsurf', 'commands');
    ...
    for (const cmd of commands) {
      // 获取模板内容（TOML 格式）
      const template = getCommandTemplate(templatesDir, 'windsurf', cmd);
      if (!template) {
        console.warn(`⚠️  模板不存在: ${cmd}`);
        continue;
      }

      // 转换格式（从 TOML 转换为 Windsurf Markdown 格式）
  ...
  transformCommand(content: string, commandName: string): string {
    ...
    // 生成 Windsurf 支持的 Markdown 格式
    // Windsurf 的命令格式：使用 YAML frontmatter + prompt
    const lines: string[] = [];
