...
describe('new command', () => {
	const testDir = join(process.cwd(), '.test-nanospec-new');
	...
	it('应该创建任务目录结构（brief.md、assets/、outputs/）', async () => {
		...
		const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
		const taskDir = join(nanospecDir, `${date}-test-task`);
	...
	it('应该创建包含任务名称的 brief.md', async () => {
		...
		const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
		const briefPath = join(nanospecDir, `${date}-用户认证功能`, 'brief.md');
	...
	it('应该正确处理包含特殊字符的任务名称', async () => {
		...
		const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
		const taskDir = join(nanospecDir, `${date}-用户登录&注册`);
